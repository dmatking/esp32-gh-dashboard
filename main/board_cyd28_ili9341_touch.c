...
static uint16_t              *s_fb_top = NULL;   // rows 0 .. FB_HALF-1
static uint16_t              *s_fb_bot = NULL;   // rows FB_HALF .. LCD_V_RES-1
static SemaphoreHandle_t      s_flush_sem = NULL;
static const char             *TAG = "BOARD_CYD28";

#define FB_HALF (LCD_V_RES / 2)   // 120 rows per half-buffer

static inline bool fb_valid(void) { return s_fb_top && s_fb_bot; }
static inline uint16_t *fb_ptr(int x, int y)
{
    return (y < FB_HALF) ? &s_fb_top[y * LCD_H_RES + x]
                         : &s_fb_bot[(y - FB_HALF) * LCD_H_RES + x];
}
...
void board_init(void)
{
    ...
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = LCD_H_RES * FB_HALF * sizeof(uint16_t) + 64,
    };
    ...
    s_fb_top = heap_caps_aligned_calloc(4, LCD_H_RES * FB_HALF * sizeof(uint16_t), 1,
                   MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    assert(s_fb_top);
    s_fb_bot = heap_caps_aligned_calloc(4, LCD_H_RES * FB_HALF * sizeof(uint16_t), 1,
                   MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
    assert(s_fb_bot);
...
{
    return -1;  // CYD has no dedicated button; use touch
}
...
{
    if (!s_panel || !fb_valid()) return;
    esp_lcd_panel_draw_bitmap(s_panel, 0, 0, LCD_H_RES, FB_HALF, s_fb_top);
    xSemaphoreTake(s_flush_sem, portMAX_DELAY);
    esp_lcd_panel_draw_bitmap(s_panel, 0, FB_HALF, LCD_H_RES, LCD_V_RES, s_fb_bot);
    xSemaphoreTake(s_flush_sem, portMAX_DELAY);
}
    ...
    if (x1 == 0 && x2 == LCD_H_RES) {
        if (y2 <= FB_HALF) {
            esp_lcd_panel_draw_bitmap(s_panel, 0, y1, LCD_H_RES, y2,
                                      &s_fb_top[y1 * LCD_H_RES]);
            xSemaphoreTake(s_flush_sem, portMAX_DELAY);
        } else if (y1 >= FB_HALF) {
            esp_lcd_panel_draw_bitmap(s_panel, 0, y1, LCD_H_RES, y2,
                                      &s_fb_bot[(y1 - FB_HALF) * LCD_H_RES]);
            xSemaphoreTake(s_flush_sem, portMAX_DELAY);
        } else {
            esp_lcd_panel_draw_bitmap(s_panel, 0, y1, LCD_H_RES, FB_HALF,
                                      &s_fb_top[y1 * LCD_H_RES]);
            xSemaphoreTake(s_flush_sem, portMAX_DELAY);
            esp_lcd_panel_draw_bitmap(s_panel, 0, FB_HALF, LCD_H_RES, y2, s_fb_bot);
            xSemaphoreTake(s_flush_sem, portMAX_DELAY);
        }
    } else {
        for (int y = y1; y < y2; y++) {
            esp_lcd_panel_draw_bitmap(s_panel, x1, y, x2, y + 1, fb_ptr(x1, y));
            xSemaphoreTake(s_flush_sem, portMAX_DELAY);
        }
    }
}
...
void board_lcd_fill(uint16_t color)
{
    ...
    int n = LCD_H_RES * FB_HALF;
    for (int i = 0; i < n; i++) s_fb_top[i] = c;
    for (int i = 0; i < n; i++) s_fb_bot[i] = c;
    board_lcd_flush();
}

void board_lcd_clear(void)
{
    if (s_fb_top) memset(s_fb_top, 0, LCD_H_RES * FB_HALF * sizeof(uint16_t));
    if (s_fb_bot) memset(s_fb_bot, 0, LCD_H_RES * FB_HALF * sizeof(uint16_t));
}

void board_lcd_set_pixel_raw(int x, int y, uint16_t color)
{
    if (fb_valid()) *fb_ptr(x, y) = color;
}

void board_lcd_set_pixel_rgb(int x, int y, uint8_t r, uint8_t g, uint8_t b)
{
    if (fb_valid()) *fb_ptr(x, y) = pack_rgb565_swapped(r, g, b);
}

...
int board_get_button_gpio(void)  { return -1; }  // CYD 3.5 has no user button

// ---------------------------------------------------------------------------
// Stripe rendering — 480x320 doesn't fit in ESP32 SRAM as one buffer; we
// expose N_STRIPES horizontal bands. board_interface callers loop
// stripe_count, call begin_stripe(s), then render their scene as normal —
// pixels outside the active stripe are silently dropped.
// ---------------------------------------------------------------------------
int  board_lcd_stripe_count(void)         { return N_STRIPES; }
void board_lcd_begin_stripe(int idx)
{
    if (idx < 0) idx = 0;
    if (idx >= N_STRIPES) idx = N_STRIPES - 1;
    s_stripe_y = idx * STRIPE_H;
}
